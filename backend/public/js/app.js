...
function showToast(msg, type) {
  ...
  el.className = 'toast-enter bg-bg-light border-2 border-txt-main shadow-md p-3 mb-2 text-sm pointer-events-auto font-medium uppercase text-xs tracking-wider rounded-md';
...
async function loadTrending() {
  ...
  try {
    ...
    wrapper.innerHTML = infiniteProducts.map(function(product) {
      ...
      var imgSrc = product.img || product.image || 'https://placehold.co/400x300/1A3636/C45C3C?text=NaturaBotanica';
  ...
  try {
    ...
    products = Array.from({ length: 65 }).map(function(_, i) {
      return {
        ...
        price: 500 + (i * 10), img: 'https://placehold.co/400x300/1A3636/C45C3C?text=Product+' + (i + 1),
...
function renderCatButtons() {
  ...
  document.getElementById('catButtons').innerHTML = cats.map(function(c) {
    ...
    return '<button onclick="filterCategory(\'' + c + '\')" class="cat-btn group p-3 border-2 border-txt-main hover:border-brand transition-all text-center rounded-lg" data-cat="' + c + '">' +
      '<div class="w-9 h-9 mx-auto mb-2 border-2 border-txt-main flex items-center justify-center bg-brand text-white rounded-full">' +
...
function renderProducts(items) {
  ...
  grid.innerHTML = pageItems.map(function(p) {
    ...
    return '<div class="product-card bg-white overflow-hidden"><div class="relative h-48 overflow-hidden cursor-pointer rounded-t-md" onclick="openModal(\'' + p.id + '\')"><img src="' + p.img + '" class="product-img w-full h-full object-cover transition-transform duration-500"><div class="absolute top-3 left-3"><span class="px-2.5 py-1 bg-white border-2 border-txt-main text-[10px] font-medium uppercase tracking-wider rounded-md">' + p.catLabel + '</span></div><div class="absolute top-3 right-3"><span class="px-2 py-0.5 bg-txt-main text-white text-[9px] font-mono font-bold rounded-md">#' + p.id + '</span></div></div><div class="p-5 border-t-2 border-txt-main"><div class="flex items-center justify-between mb-1"><h3 class="text-base font-semibold cursor-pointer hover:text-brand uppercase tracking-wider text-sm font-serif" onclick="openModal(\'' + p.id + '\')">' + p.name + '</h3>' + stockBadge + '</div><div class="flex items-center justify-between mt-2"><div><span class="text-xl font-semibold">NPR ' + p.price.toLocaleString() + ' <span class="text-xs font-normal lowercase">/ ' + p.unit + '</span></span><span class="text-xs text-txt-sub block">$' + priceUSD + '</span></div><button onclick="addToCart(\'' + p.id + '\')" ' + disabledAttr + ' class="px-4 py-2 ' + btnClass + ' text-bg-light text-xs uppercase tracking-wider transition-all border-2 border-txt-main shadow-md rounded-md">' + btnText + '</button></div></div></div>';
  }).join('');
...
function renderPagination(totalPages) {
  ...
  var pagesHtml = '<button onclick="goToPage(' + (currentPage - 1) + ')" ' + (currentPage === 1 ? 'disabled' : '') + ' class="flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-2 border-txt-main ' + (currentPage === 1 ? 'opacity-40 cursor-not-allowed' : 'hover:bg-txt-main hover:text-bg-light') + ' transition-all uppercase text-xs rounded-md"><i data-lucide="chevron-left" class="w-4 h-4"></i>Prev</button>';
  var pageNumbers = [];
  ...
  pageNumbers.forEach(function(p) {
    ...
    else { var isActive = p === currentPage; pagesHtml += '<button onclick="goToPage(' + p + ')" class="w-10 h-10 flex items-center justify-center text-sm font-medium border-2 border-txt-main transition-all rounded-md ' + (isActive ? 'bg-txt-main text-bg-light shadow-md' : 'hover:bg-brand hover:text-white') + '">' + p + '</button>'; }
  });
  pagesHtml += '<button onclick="goToPage(' + (currentPage + 1) + ')" ' + (currentPage === totalPages ? 'disabled' : '') + ' class="flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-2 border-txt-main ' + (currentPage === totalPages ? 'opacity-40 cursor-not-allowed' : 'hover:bg-txt-main hover:text-bg-light') + ' transition-all uppercase text-xs rounded-md">Next<i data-lucide="chevron-right" class="w-4 h-4"></i></button>';
  container.innerHTML = '<div class="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 pt-6 border-t-2 border-txt-main"><p class="text-sm text-txt-third uppercase text-xs tracking-wider">Page <span class="font-semibold text-txt-main">' + currentPage + '</span> of <span class="font-semibold text-txt-main">' + totalPages + '</span></p><div class="flex items-center gap-2 flex-wrap justify-center">' + pagesHtml + '</div></div>';
  lucide.createIcons();
}
...
function initSearch() {
  document.getElementById('searchInput').addEventListener('input', function(e) {
    ...
    resultsDiv.innerHTML = matches.map(function(p) {
      return '<div class="flex items-center gap-3 p-3 hover:bg-bg-light cursor-pointer transition-colors border-b-2 border-txt-main/20 rounded-md" onclick="closeSearch(); openModal(\'' + p.id + '\');"><img src="' + p.img + '" class="w-10 h-10 object-cover border border-txt-main rounded-md" onerror="this.style.display=\'none\'"><div class="flex-1 min-w-0"><p class="text-sm font-medium truncate uppercase text-xs">' + p.name + '</p><p class="text-[10px] text-txt-third uppercase">' + p.catLabel + ' · NPR ' + p.price.toLocaleString() + '</p></div><i data-lucide="arrow-up-right" class="w-4 h-4 text-txt-third flex-shrink-0"></i></div>';
    }).join('');
    lucide.createIcons();
  });
...
function openModal(id) {
  ...
  var html = '<div class="relative"><img src="' + p.img + '" class="w-full h-44 object-cover rounded-t-xl" onerror="this.style.display=\'none\'"><button onclick="closeModal()" class="absolute top-2 right-2 w-7 h-7 bg-white border-2 border-txt-main flex items-center justify-center shadow-md rounded-full"><i data-lucide="x" class="w-3.5 h-3.5"></i></button><div class="absolute top-2 left-2 px-2 py-0.5 bg-brand text-white text-[9px] font-medium uppercase tracking-wider border-2 border-txt-main rounded-md">' + p.catLabel + '</div><div class="absolute bottom-2 right-2 px-2 py-0.5 bg-txt-main text-white text-[8px] font-mono font-bold rounded-md">#' + p.id + '</div></div><div class="p-4"><div class="flex justify-between items-start gap-2 mb-1"><h3 class="text-lg font-semibold tracking-tight uppercase font-serif">' + p.name + '</h3><span class="text-[10px] font-mono bg-brand/10 px-2 py-0.5 border-2 border-txt-main text-brand font-bold rounded-md">#' + p.id + '</span></div><p class="text-[10px] text-txt-third italic mb-2">' + (p.sci || '') + '</p><p class="text-[11px] text-txt-sub leading-relaxed mb-3 line-clamp-2">' + (p.desc || '') + '</p><div class="grid grid-cols-2 gap-2 mb-3"><div class="fancy-select relative" id="formDDCont"><div class="flex items-center justify-between p-2 border-2 border-txt-main bg-white cursor-pointer text-xs rounded-md" onclick="toggleDropdown(\'form\')"><div class="flex items-center gap-1.5"><i data-lucide="' + (iconMap[selectedForm] || 'layers') + '" class="w-3 h-3 text-brand"></i><span class="text-[11px] font-medium uppercase" id="selFormLabel">' + selectedForm + '</span></div><i data-lucide="chevron-down" class="w-3 h-3 transition-transform" id="formChev"></i></div><div class="fancy-select-dropdown" id="formDD">' + formOptions.map(function(opt) { return '<div class="fancy-option text-xs py-1.5 px-3" onclick="selectForm(\'' + opt + '\')"><i data-lucide="' + (iconMap[opt] || 'layers') + '" class="w-3 h-3"></i><span class="uppercase">' + opt + '</span></div>'; }).join('') + '</div></div><div class="fancy-select relative" id="unitDDCont"><div class="flex items-center justify-between p-2 border-2 border-txt-main bg-white cursor-pointer text-xs rounded-md" onclick="toggleDropdown(\'unit\')"><div class="flex items-center gap-1.5"><i data-lucide="scale" class="w-3 h-3 text-brand"></i><span class="text-[11px] font-medium uppercase" id="selUnitLabel">' + selectedUnitValue + '</span></div><i data-lucide="chevron-down" class="w-3 h-3 transition-transform" id="unitChev"></i></div><div class="fancy-select-dropdown" id="unitDD">' + unitOptions.map(function(opt) { return '<div class="fancy-option text-xs py-1.5 px-3" onclick="selectUnit(\'' + opt + '\')"><i data-lucide="' + (opt.includes('ml') ? 'droplet' : 'cube') + '" class="w-3 h-3"></i><span>' + opt + '</span></div>'; }).join('') + '</div></div></div><div class="bg-brand/5 p-2.5 border-2 border-txt-main mb-3 rounded-lg"><div class="flex justify-between items-center"><div><span class="text-[9px] uppercase text-txt-sub font-bold">Total Price</span><div class="text-lg font-bold text-brand" id="dynPrice">NPR 0</div><div class="text-[9px] text-txt-sub" id="dynUsd">$0.00</div><div id="powderNote" class="text-[8px] text-orange-500 mt-1 font-bold uppercase ' + (selectedForm === 'Powder Form' ? '' : 'hidden') + '">+NPR 50/kg powder fee included</div></div><div class="text-right"><span class="text-[8px] bg-brand/10 px-1.5 py-0.5 border border-txt-main uppercase font-bold rounded-sm" id="mFormBadge">' + selectedForm + '</span><span class="text-[8px] bg-brand/10 px-1.5 py-0.5 border border-txt-main ml-1 uppercase font-bold rounded-sm" id="mUnitBadge">' + selectedUnitValue + '</span></div></div></div><div class="grid grid-cols-3 gap-1.5 mb-3"><div class="p-1.5 bg-bg-dark2 text-center border-2 border-txt-main rounded-md"><p class="text-[8px] text-white/50 mb-0.5 uppercase">Base Unit</p><p class="text-[10px] text-bg-light font-medium">' + (p.unit || '1 kg') + '</p></div><div class="p-1.5 bg-bg-dark2 text-center border-2 border-txt-main rounded-md"><p class="text-[8px] text-white/50 mb-0.5 uppercase">MOQ</p><p class="text-[10px] text-bg-light font-medium">' + (p.moq || '1 kg') + '</p></div><div class="p-1.5 bg-bg-dark2 text-center border-2 border-txt-main rounded-md"><p class="text-[8px] text-white/50 mb-0.5 uppercase">Lead</p><p class="text-[10px] text-bg-light font-medium">' + (p.lead || '7d') + '</p></div></div><div class="flex gap-2"><div class="flex items-center border-2 border-txt-main overflow-hidden rounded-md"><button onclick="changeQty(-1)" class="px-2.5 py-1.5 hover:bg-txt-main hover:text-bg-light"><i data-lucide="minus" class="w-3 h-3"></i></button><input id="modalQty" type="number" value="1" min="1" class="w-10 text-center py-1.5 text-xs font-medium border-x-2 border-txt-main focus:outline-none"><button onclick="changeQty(1)" class="px-2.5 py-1.5 hover:bg-txt-main hover:text-bg-light"><i data-lucide="plus" class="w-3 h-3"></i></button></div><button onclick="addEnhancedToCart()" class="flex-1 py-1.5 bg-brand text-white text-xs font-medium hover:bg-brand-hover flex items-center justify-center gap-1.5 transition-all uppercase border-2 border-txt-main shadow-brand rounded-md"><i data-lucide="shopping-bag" class="w-3.5 h-3.5"></i> Add to Cart</button></div></div>';
