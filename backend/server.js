...
// ─── BREVO EMAIL: CLIENT STATUS UPDATE ────────────────────────────────────────
...
// ─── BREVO EMAIL: ADMIN NEW ORDER ALERT ────────────────────────────────────────
...
// ─── ROUTE 3: NEW ORDER (NO STOCK DEDUCTION) ────────────────────────────────
...
// ─── ROUTE 7: STOCK MANAGER DASHBOARD (PROTECTED) ────────────────────────────────
    ...
    res.render(`
      ...
      <style>
        *{box-sizing:border-box}body{font-family:-apple-system,sans-serif;background:#f3f4f6;margin:0;padding:20px;color:#1f2937}
        ...
        .si:focus{outline:none;border-color:#A3B14B}.sb{background:#A3B14B;color:#fff;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-weight:600;font-size:12px;opacity:0;transition:.2s}.sv{opacity:1}
        .toast{position:fixed;bottom:30px;left:50%;transform:translateX(-50%) translateY(20px);background:#1f2937;color:#fff;padding:12px 24px;border-radius:50px;opacity:0;transition:.3s;z-index:2000}.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}
      ...
      <script>
        ...
        async function save(id) {
          ...
          try {
            const r = await fetch(baseUrl+'/update-stock', {method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id,stock:v})});
