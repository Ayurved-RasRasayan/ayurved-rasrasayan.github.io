...
const jwt = require('jsonwebtoken'); // FIXED HERE
...
const JWT_SECRET = process.env.JWT_SECRET; // No weak fallback
...
// Optional auth: extracts user if token exists, but doesn't block unauthenticated requests
...
async function sendClientEmail(toEmail, toName, orderId, status) {
  try {
    ...
    await axios.post('https://api.brevo.com/v3/smtp/email', {
      ...
      to: [{ email: toEmail, name: escapeHtml(toName) }],
      subject: `Order Update: #${orderId}`,
      htmlContent: `<h3>Hello ${escapeHtml(toName)},</h3><p>Your order #${orderId} is now: <strong>${escapeHtml(displayStatus)}</strong>.</p><p>Thank you!</p>`
...
async function sendAdminAlert(orderId, data) {
  try {
    const safeName = escapeHtml(data.clientDetails.name);
    ...
    (data.items || []).forEach(i => { 
      itemsHtml += `<tr><td style="border:1px solid #e5e7eb;padding:8px;">${escapeHtml(i.name)}</td><td style="border:1px solid #e5e7eb;padding:8px;">${escapeHtml(i.form || 'N/A')}</td><td style="border:1px solid #e5e7eb;padding:8px;">${escapeHtml(i.unit || 'N/A')}</td><td style="border:1px solid #e5e7eb;padding:8px;text-align:center;">${parseInt(i.qty)||1}</td><td style="border:1px solid #e5e7eb;padding:8px;">$${i.price||0}</td></tr>`; 
    });
    itemsHtml += '</table>';
    ...
    await axios.post('https://api.brevo.com/v3/smtp/email', {
      ...
      subject: `🛒 NEW ORDER: #${orderId} - ${safeName}`,
      htmlContent: `<div style="font-family:Arial;color:#333;"><h2 style="color:#2d4a22;">New Order (#${orderId})</h2><p><b>Name:</b> ${safeName}<br><b>Email:</b> ${escapeHtml(data.clientDetails.email)}<br><b>Phone:</b> ${escapeHtml(data.clientDetails.phone)}</p><p><b>Total:</b> $${data.totalUSD} (${data.totalNPR} NPR)</p>${itemsHtml}${data.paymentScreenshot ? `<p>📸 <a href="${data.paymentScreenshot}" target="_blank">View Screenshot</a></p>` : ''}</div>`
    }, { headers: { 'api-key': process.env.EMAIL_PASS, 'content-type': 'application/json' } });
...
async function sendInquiryAlert(data) {
  try {
    ...
    await axios.post('https://api.brevo.com/v3/smtp/email', {
      ...
      htmlContent: `<div style="font-family:Arial;color:#333;padding:20px;border:1px solid #eee;"><h2 style="color:#A3B14B;">New Inquiry Received</h2><p style="background:#f9fafb;padding:10px;border-radius:5px;"><strong>Name:</strong> ${fullName}<br><strong>Email:</strong> ${escapeHtml(data.email)}<br><strong>Company:</strong> ${escapeHtml(data.company || 'N/A')}</p><div style="margin-top:20px;"><strong>Message:</strong><p style="background:#fff;padding:15px;border:1px solid #eee;margin-top:5px;">${escapeHtml(data.message)}</p></div></div>`
    }, { headers: { 'api-key': process.env.EMAIL_PASS, 'content-type': 'application/json' } });
    return true;
...
async function sendOTPEmail(toEmail, toName, otp) {
  try {
    await axios.post('https://api.brevo.com/v3/smtp/email', {
      ...
      htmlContent: `<div style="font-family:Arial;color:#333;text-align:center;padding:40px;"><h2 style="color:#2d4a22;">Email Verification</h2><p>Hello ${escapeHtml(toName)},</p><p>Your verification code is:</p><h1 style="font-size:40px;color:#A3B14B;letter-spacing:5px;">${otp}</h1><p>This code expires in 10 minutes.</p></div>`
    }, { headers: { 'api-key': process.env.EMAIL_PASS, 'content-type': 'application/json' } });
    return true;
...
const orderSchema = new mongoose.Schema({ 
  items: Array, totalUSD: Number, totalNPR: Number, paidAmount: Number, currency: String, paymentMethod: String, paymentScreenshot: String, 
  clientDetails: { name: String, phone: String, email: String, address: String }, 
  userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', default: null }, // Added explicit ownership
  status: { type: String, default: 'Pending' }, order_state: { type: String, default: 'Pending' }, emailStatus: { type: String, default: 'Queue' }, timestamp: { type: Date, default: Date.now } 
});
...
visitorSchema.index({ lastVisited: 1 }, { expireAfterSeconds: 86400 }); // TTL: auto-delete IPs after 24h
...
async function syncProductsToDB(productsArray, { removeOrphans = false, resetStock = false } = {}) {
  ...
  for (const p of productsArray) { 
    ...
    if (existing) { 
      const updateData = { ...p }; 
      if (!resetStock) updateData.stock = existing.stock; 
  ...
  if (removeOrphans) { 
    ...
    for (const dbp of dbProducts) { 
      if (!incomingIds.has(dbp.id)) { 
        await Product.deleteOne({ id: dbp.id }); removed++; 
      } 
    } 
  }
...
function startFileWatcher() { 
  try { 
    watcher = fs.watch(PRODUCTS_FILE, (eventType) => { 
...
app.post('/api/auth/resend-otp', authLimiter, asyncHandler(async (req, res) => {
...
app.get('/api/auth/me', userAuth, asyncHandler(async (req, res) => {
  const user = await User.findById(req.user.id).select('-password -verificationCode -verificationExpires'); 
  if (!user) return res.status(404).json({ error: 'User not found' }); 
  res.json(user);
}));

app.post('/api/cart/sync', userAuth, asyncHandler(async (req, res) => {
  ...
  // Trust client state (replace entirely to allow deletions/qty adjustments)
  user.cart = localCart; 
  await user.save(); 
  res.json({ success: true, cart: user.cart });
}));

app.get('/api/auth/my-orders', userAuth, asyncHandler(async (req, res) => {
  // Secure query by userId instead of just email
  const orders = await Order.find({ userId: req.user.id }).sort({ timestamp: -1 }); 
  res.json(orders);
}));

// ─── ADMIN USER MANAGEMENT ──────────────────────────────────────────────
app.get('/api/users-data', checkAuth, asyncHandler(async (req, res) => {
  const users = await User.find().sort({ _id: -1 }).select('-password -verificationCode -verificationExpires'); 
  res.json({ success: true, users });
}));

app.delete('/api/delete-user/:id', checkAuth, asyncHandler(async (req, res) => {
  await User.findByIdAndDelete(req.params.id); 
  res.json({ success: true });
}));

app.get('/api/manage-users', checkAuth, asyncHandler((req, res) => {
  let html = fs.readFileSync(path.join(__dirname, 'views/user-management.html'), 'utf8'); 
  res.send(html);
}));
...
app.get('/api/public/rate', asyncHandler(async (req, res) => { 
  const rate = await getSetting('exchange_rate', 133); 
  res.json({ rate }); 
}));

app.get('/api/exchange-rate/fetch', checkAuth, asyncHandler(async (req, res) => { 
  const rate = await syncExchangeRate(); 
  if (rate) res.json({ success: true, rate }); 
  else res.status(502).json({ error: 'Failed' }); 
}));

app.get('/api/public/visits', asyncHandler(async (req, res) => { 
  const ip = getClientIp(req); 
  await Visitor.updateOne({ ip }, { $set: { lastVisited: new Date() } }, { upsert: true }); 
  const baseCount = await getSetting('base_visitor_count', 5000); 
  const uniqueCount = await Visitor.countDocuments({}); 
  res.json({ count: baseCount + uniqueCount }); 
}));
...
app.get('/api/view-orders-data', checkAuth, asyncHandler(async (req, res) => { 
...
app.get('/api/products', asyncHandler(async (req, res) => { 
  ...
  const [products, total] = await Promise.all([
    Product.find().sort({ id: 1 }).skip(skip).limit(limit).lean(),
    Product.countDocuments()
  ]);
  
  res.json({ 
    products: products.map(({ _id, __v, ...rest }) => rest), 
    page, 
...
app.post('/api/products', checkAuth, asyncHandler(async (req, res) => { 
  const { id, name, price } = req.body; 
  if (!id || !name || price === undefined) return res.status(400).json({ error: 'Required' }); 
  if (await Product.findOne({ id })) return res.status(409).json({ error: 'Exists' }); 
  const p = new Product({ ...req.body, stock: req.body.stock ?? 100 }); 
  await p.save(); await syncDBToFile(); 
  res.status(201).json({ success: true }); 
}));

app.put('/api/products/:id', checkAuth, asyncHandler(async (req, res) => { 
  const updateData = { ...req.body }; 
  if (req.body.stock === undefined) delete updateData.stock; 
  await Product.updateOne({ id: Number(req.params.id) }, { $set: updateData }); 
  await syncDBToFile(); 
  res.json({ success: true }); 
}));

app.delete('/api/products/:id', checkAuth, asyncHandler(async (req, res) => { 
...
app.post('/api/products/bulk', checkAuth, asyncHandler(async (req, res) => { 
  if (!Array.isArray(req.body.products)) return res.status(400).json({ error: 'Array required' }); 
  const result = await syncProductsToDB(req.body.products, { removeOrphans: false }); 
  await syncDBToFile(); 
  res.json({ success: true, ...result }); 
}));

app.post('/api/seed', checkAuth, asyncHandler(async (req, res) => { 
  if (req.body.confirm !== 'RESET_ALL_PRODUCTS') {
    return res.status(400).json({ error: 'Confirmation required. Body must include { "confirm": "RESET_ALL_PRODUCTS" }' });
  }
  await Product.deleteMany({}); 
  await Product.insertMany(require('./products.json')); 
  res.json({ success: true }); 
}));

app.get('/api/sync-products', checkAuth, asyncHandler(async (req, res) => { 
  const result = await syncFileToDB({ removeOrphans: true }); 
  res.json({ success: true, ...result }); 
}));

app.get('/api/seed-stock', checkAuth, asyncHandler(async (req, res) => { 
  await Product.updateMany({}, { $set: { stock: 100 } }); 
  res.json({ success: true, message: 'Stocks reset' }); 
}));

// ─── ORDERS ─────────────────────────────────────────────────────────────
app.post('/api/orders', orderLimiter, optionalUserAuth, asyncHandler(async (req, res) => { 
  ...
  for (const item of req.body.items) {
    ...
    if (product.stock < requestedQty) {
      return res.status(400).json({ success: false, error: `Insufficient stock for "${item.name}" (available: ${product.stock})` });
    }
  }

  // Add userId if user is authenticated
  const orderData = { ...req.body, order_state: 'Pending', userId: req.user?.id || null };
...
app.put('/api/update-status', checkAuth, asyncHandler(async (req, res) => { 
  ...
  if (shouldDeduct && !wasDeducted) { 
    for (const item of (order.items || [])) {
      ...
      // Atomic guard: prevent stock from going negative
      const result = await Product.updateOne({ id: item.id, stock: { $gte: qty } }, { $inc: { stock: -qty } });
      if (result.matchedCount === 0) console.warn(`⚠️ Stock insufficient for product ${item.id} during status update.`);
    }
...
app.put('/api/update-order-state', checkAuth, asyncHandler(async (req, res) => { 
  const { id, state } = req.body; 
  if (!id || !state) return res.status(400).json({ success: false }); 
  await Order.updateOne({ _id: id }, { $set: { order_state: state } }); 
  res.json({ success: true }); 
}));

app.delete('/api/delete-order/:id', checkAuth, asyncHandler(async (req, res) => { 
  await Order.findByIdAndDelete(req.params.id); 
  res.json({ success: true }); 
}));

app.delete('/api/delete-orders', checkAuth, asyncHandler(async (req, res) => { 
  if (!Array.isArray(req.body.ids)) return res.status(400).json({ success: false, error: 'ids array required' });
...
app.post('/api/inquiries', asyncHandler(async (req, res) => { 
  if (!req.body.email || !req.body.message) return res.status(400).json({ error: 'Missing' }); 
...
// ─── STOCK MANAGEMENT UI ───────────────────────────────────────────────
app.put('/api/update-stock', checkAuth, asyncHandler(async (req, res) => { 
  if (typeof req.body.stock !== 'number' || req.body.stock < 0) return res.status(400).json({ error: 'Invalid stock value' });
  await Product.updateOne({ id: req.body.id }, { $set: { stock: req.body.stock } }); 
  await syncDBToFile(); 
  res.json({ success: true }); 
}));

app.get('/api/manage-stock', checkAuth, asyncHandler(async (req, res) => { 
  const products = await Product.find().sort({ id: 1 }); 
  let rows = products.map(p => { 
    const s = p.stock || 0; 
    let badge = s === 0 ? '<span style="color:#b91c1c">Out</span>' : s <= 10 ? '<span style="color:#c2410c">Low</span>' : '<span style="color:#15803d">In Stock</span>'; 
    return `<tr><td><strong>${escapeHtml(p.name)}</strong></td><td>${escapeHtml(p.catLabel)}</td><td>${badge} (${s})</td><td><input type="number" id="s-${p.id}" value="${s}" min="0"></td><td><button onclick="save(${p.id})">Save</button></td></tr>`; 
  }).join(''); 
  
  // Fixed: fetch() syntax corrected, removed auth header leak (browser auto-includes Basic Auth for same-origin)
  res.send(`<!DOCTYPE html><html><head><style>body{font-family:sans-serif;padding:20px}table{border-collapse:collapse;width:100%}td,th{border:1px solid #ddd;padding:8px;text-align:left}button{cursor:pointer;padding:5px 10px}</style></head><body><h2>Stock Management</h2><table><tr><th>Product</th><th>Category</th><th>Status</th><th>New Stock</th><th>Action</th></tr>${rows}</table><script>async function save(id){const v=document.getElementById('s-'+id).value;await fetch('/api/update-stock',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({id:id,stock:+v})});location.reload();}</script></body></html>`); 
}));

app.get('/api/view-orders', checkAuth, asyncHandler((req, res) => { 
  let html = fs.readFileSync(path.join(__dirname, 'views/orders.html'), 'utf8'); 
  res.send(html); 
}));

app.get('/profile', (req, res) => { 
  try { let html = fs.readFileSync(path.join(__dirname, 'views/user.html'), 'utf8'); res.send(html); } 
  catch (e) { res.status(500).send('Error loading profile page'); } 
});
